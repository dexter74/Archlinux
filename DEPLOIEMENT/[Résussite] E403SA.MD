...
#### 2. Configuration Réseau (Mode Automatique)
```shell
clear;
...
#### 2. Configuration Réseau (Manuel)

Pour la valeur Name on peut utiliser le joker. (wl*, et*, br*)
...
Type=local " > /etc/systemd/network/1-lo.network ;
```
...
```
echo "[Match]
Name=$(ip link | grep wlan | cut -c 4-7)
...
echo "journalctl -u systemd-resolved"  > resolved.sh;
echo "journalctl -u systemd-networkd"  > networkd.sh;
echo "journalctl -u systemd-timesyncd" > timesyncd.sh;
echo "journalctl -u iwd"               > iwd.sh;
```
------------------------------------------------------------------
#### Post Démarrage
```shell
clear;
#Gestion Service:
sh services.sh {enable,start,status}
