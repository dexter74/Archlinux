...
clear;
cat /etc/systemd/network/2-$(ip link | grep wlan | cut -c 4-7).network;' > wlan_dhcp.sh;
```
...
clear;
cat /etc/systemd/network/2-$(ip link | grep wlan | cut -c 4-7).network;' > wlan_static.sh;
```
