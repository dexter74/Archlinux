...
```bash
clear;
systemctl --user --now enable  {pipewire.socket,pipewire.service,pulseaudio};
systemctl --user --now disable {pipewire.socket,pipewire.service,pulseaudio};
systemctl reboot;
```
